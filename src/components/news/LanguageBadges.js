      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {allLocales.map(locale => {
          ...
          return (
            <span
              key={locale}
              className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${isAvailable
                  ? isCurrent
            ...
            <div className="flex gap-2 flex-wrap">
              {availableLocales.map(locale => {
                ...
                return (
                  <span
                    key={locale}
                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-base ${isCurrent
                        ? 'bg-brand-accent text-white font-semibold'
